...
const Analytics: React.FC<Props> = ({ history, language, daysSkating = 1, user, onLogin }) => {
  ...
  if (!user) {
    ...
    return (
        <div className="flex flex-col h-full p-6 items-center justify-center space-y-6 animate-fade-in relative overflow-hidden">
             {/* Background Effects */}
             <div className="absolute top-0 left-1/4 w-96 h-96 bg-skate-neon/5 rounded-full filter blur-[100px] animate-blob"></div>
             
             <div className="glass-card p-8 rounded-3xl w-full max-w-sm text-center flex flex-col items-center shadow-2xl border border-white/10 z-10">
